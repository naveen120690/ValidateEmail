    ...
    public class ImapConnector
    {
        ...
        public bool Login(string host, int port, string username, string password, NGConnectionSecurity connectionSecurity)
        {
            try
            {
                ...
                if (!this.CheckResponse(NGImapStatusCode.IMAP_OK))
                {
                    Dispose();
                    return false;
                }
                ...
                this.Write(string.Format(NGImapCommands.CAPABILITY_CMD, NGImapCommands.PREFIX_CMD, NGImapCommands.EOL));
                if (!this.CheckResponse(NGImapStatusCode.IMAP_OK_COMPLETED))
                {
                    Dispose();
                    return false;
                }
                ...
                {
                    this.Write(string.Format(NGImapCommands.STARTTLS_CMD, NGImapCommands.PREFIX_CMD, NGImapCommands.EOL));
                    if (!this.CheckResponse(NGImapStatusCode.IMAP_OK_COMPLETED))
                    {
                        Dispose();
                        return false;
                ...
                this.Write(string.Format(NGImapCommands.LOGIN_CMD, NGImapCommands.PREFIX_CMD,username,password,NGImapCommands.EOL));
                if (!this.CheckResponse(NGImapStatusCode.IMAP_OK_COMPLETED))
                {
                    Dispose();
                    return false;
        ...
        private bool CheckResponse(string expectedCode)
        {
            ...
            try
            {
                ...
                while (bRead)
                {
                    ...
                    else if (strServerResponse.StartsWith(NGImapStatusCode.IMAP_NO))
                    {
                        bRead = false;
                    }
                    else if (strServerResponse.StartsWith(NGImapStatusCode.IMAP_BAD))
                    {
                        bRead = false;
                    }
                    else
